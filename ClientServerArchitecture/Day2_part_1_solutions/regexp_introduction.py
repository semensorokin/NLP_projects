...
def match_pavel_sukhov(text):
    expr = re.compile("(P|p)avel (S|s)ukhov$")
    return bool(expr.match(text))
...
def match_alphabet_order(text):
    expr = re.compile("^a?[^a-z]*b?[^a-z]*c?[^a-z]*d?[^a-z]*e?[^a-z]*f?[^a-z]*g?[^a-z]*h?[^a-z]*i?[^a-z]*j?[^a-z]*k?[^a-z]*l?[^a-z]*m?[^a-z]*n?[^a-z]*o?[^a-z]*p?[^a-z]*q?[^a-z]*r?[^a-z]*s?[^a-z]*t?[^a-z]*u?[^a-z]*v?[^a-z]*w?[^a-z]*x?[^a-z]*y?[^a-z]*z?[^a-z]*$")
    return bool(expr.match(text))
...
def match_ip_address(text):
    expr = re.compile("^((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])$")
    return bool(expr.match(text))
...
"""
def match_date(text):
    expr = re.compile("^(((((0[1-9])|(1\d)|(2[0-8]))\/((0[1-9])|(1[0-2])))|((31\/((0[13578])|(1[02])))|((29|30)\/((0[1,3-9])|(1[0-2])))))\/((20[0-9][0-9])|(19[0-9][0-9])))|((29\/02\/(19|20)(([02468][048])|([13579][26]))))$")
    return bool(expr.match(text))
...
def parse_http_url(text):
    expr = re.compile("^((https?):)?(//(([^\.    '_:/\\?#-]([^\.    '_:/\\?#]*[^\.    '_:/\\?#-])?(\.[^\.    '_:/\\?#-]([^\.    '_:/\\?#]*[^\.    '_:/\\?#-])?)*)(?::([^/\\?#]*))?))(/[^\\?#]*)?(\\?([^#]*))?(#(.*))?$")
    res = expr.match(text)
    if res and res.group(2) and (not res.group(9) or 0 <= int(res.group(9)) <= 65535 ):
        return (res.group(2),
                res.group(5),
                int(res.group(9)) if res.group(9) else 80,
                res.group(10) if res.group(10) else '/',
                res.group(12) if res.group(12) else '')
    else:
        return None
