...
def construct_names(names):
    return json.dumps({
        "names": filter(lambda n: n[0].isupper(), names)
    })
...
def group_strings(words):
    return json.dumps(
        reduce(lambda x, y: x.update({y[0]: x.get(y[0], []) + [y]}) or x, words, {})
    )
...
def extract_long_names(resp):
    return filter(lambda x: len(x) > 5, resp.get("names", []))
...
def count_words(text):
    return reduce(lambda x, y: x.update({y: x.get(y, 0) + 1}) or x,
